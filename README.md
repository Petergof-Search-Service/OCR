...
OCR API имеет квоту RPS на каталог. Защита от 429 — в два слоя:

- **Per-file:** token-bucket `OCR_MAX_RPS` (дефолт 8) + экспоненциальный backoff с `Retry-After`.
  Ограничивает нагрузку одного файла/инстанса.
- **Глобально:** поскольку каждый файл — отдельный вызов функции, общий RPS ограничивается
  числом одновременных инстансов через scaling policy
  (`zone-instances-limit=2`, тег `$latest`). Сеть на 3 зонах ⇒ ≤ 6 инстансов ⇒
  пик ≈ `6 · OCR_MAX_RPS`.

Тюнинг — по логам (`Pages summary` и счётчик `429`): два регулятора — `zone-instances-limit`
и `OCR_MAX_RPS`. Потеря страниц (таймаут/ошибка) сверх `OCR_MAX_FAILED_PAGES` (дефолт 0) переводит
файл в `failed`, чтобы неполный документ не попал в индекс. Подробности и подводные камни (риск
дропнутых событий триггера при низком лимите) — в [`CLAUDE.md`](CLAUDE.md).
