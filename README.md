...
src/ocr/              # пакет пайплайна распознавания:
  pipeline.py         #   оркестратор YandexOCRAsync (process_pdf/chunk/batch)
  client.py           #   OCR API: submit + поллинг (два раздельных RPS-лимитера)
  render.py           #   рендер страниц в image-PDF (byte-aware под лимит 10 МБ)
  overlay.py          #   текстовый слой (searchable PDF) + парсинг ответа OCR
  pdf_split.py        #   нарезка на чанки/батчи
  pdf_merge.py        #   слияние постраничных PDF (внешняя утилита + pypdf-фолбэк)
  outputs.py          #   ResultWriter: сборка txt/json/pdf
  layout.py / fsutil.py / ratelimit.py
...
## Масштабирование и лимиты OCR API

OCR API имеет **раздельные** квоты RPS: submit `recognizeTextAsync` ≈10 rps, поллинг
`getRecognition` ≈50 rps; плюс ≤200 стр и ≤10 МБ на запрос. Защита от 429 и ускорение:

- **Раздельные лимитеры:** `OCR_SUBMIT_RPS` (дефолт 8) и `OCR_POLL_RPS` (дефолт 40) +
  экспоненциальный backoff с `Retry-After`. Поллинг больше не душит отправку.
- **Много страниц на запрос:** `OCR_BATCH_SIZE` (дефолт 10) кратно сокращает число submit-запросов;
  `render` режет батч по `OCR_MAX_BATCH_BYTES` (~9 МБ), чтобы не превысить лимит 10 МБ.
- **Глобально:** общий RPS между инстансами ограничивается scaling policy
  (`zone-instances-limit=2`, тег `$latest`): ≤6 инстансов ⇒ пик ≈ `6 · OCR_SUBMIT_RPS`.

После поднятия квоты — поднять `OCR_SUBMIT_RPS` (и при желании `OCR_BATCH_SIZE`), код менять не
нужно. Тюнинг — по логам (`Pages summary`, счётчики `poll HTTP 429` / `recognize … HTTP 429`).
Потеря страниц сверх `OCR_MAX_FAILED_PAGES` (дефолт 0) переводит файл в `failed`. Подробности и
подводные камни — в [`CLAUDE.md`](CLAUDE.md).
